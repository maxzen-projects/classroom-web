...
const StudentAcademics = () => {
  ...
  const groupedBySubject = React.useMemo(() => {
    ...
    return marks.reduce((acc, mark) => {
      const subjectName = mark.examId?.subjectId?.name || 'Unassigned';
      if (!acc[subjectName]) {
        acc[subjectName] = [];
      }
  ...
  return (
    <RoleProtectedRoute allowedRoles={[ROLES.STUDENT]}>
      <div className="p-6 space-y-8">
        ...
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-gray-900">Exam Results by Subject</h2>
          
          {marks && marks.length > 0 ? (
            <div className="space-y-4">
              {Object.entries(groupedBySubject).map(([subjectName, subjectMarks]) => {
                const isExpanded = expandedSubjects[subjectName] !== false; // Default to expanded
                const subjectAverage = subjectMarks.reduce((sum, mark) => {
                  const percentage = (mark.marksObtained / mark.examId?.totalMarks) * 100;
                  return sum + percentage;
                }, 0) / subjectMarks.length;

                return (
                  <div key={subjectName} className="bg-white rounded-lg shadow-sm border overflow-hidden">
                    <button
                      onClick={() => toggleSubject(subjectName)}
                      className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors border-b"
                    >
                      <div className="flex items-center gap-4 flex-1">
                        <div className={`text-2xl font-bold ${subjectAverage >= 60 ? 'text-green-600' : 'text-red-600'}`}>
                          {subjectAverage.toFixed(1)}%
                        </div>
                        <div className="text-left">
                          <h3 className="text-lg font-semibold text-gray-900">{subjectName}</h3>
                          <p className="text-sm text-gray-600">{subjectMarks.length} exam{subjectMarks.length !== 1 ? 's' : ''}</p>
                        </div>
                      </div>
                      <div>
                        {isExpanded ? (
